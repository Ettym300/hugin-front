...
const URL = "https://supertiger.hugin.app/trackdispresence";
const HUGIN_APP_ID = "1630300334100500480";
...
export const useDiscordActivityTracker = () => {
  const start = () => {
    ...
    const handleActivity = debounce((data: FormattedPresence) => {
      ...
      const mapped = activities.map((activity) => {
        ...
        return {
          id: activity.syncId || activity.applicationId || HUGIN_APP_ID,
          data: {
            startedAt:
  ...
  const restart = () => {
    ...
    localRPC.updateRPC(HUGIN_APP_ID);
    clearInterval(pingIntervalId);
    if (ws) {
      ws.onclose = () => {};
      ws.close();

...
let huginUploadsFolder: gapi.client.drive.File | undefined;

export const getOrCreateUploadsFolder = async (accessToken: string) => {
  if (huginUploadsFolder) return huginUploadsFolder;
  if (!googleApiInitialized()) await initializeGoogleDrive(accessToken);
  const res = await gapi.client.drive.files.list({
    q: "name = 'hugin_uploads' and mimeType = 'application/vnd.google-apps.folder'",
    fields: "files(id)"
  });
  const folder = res.result.files?.[0];
  if (folder) {
    huginUploadsFolder = folder;
    return huginUploadsFolder;
  }
  ...
  huginUploadsFolder = newFolder.result;
  return huginUploadsFolder;
};
